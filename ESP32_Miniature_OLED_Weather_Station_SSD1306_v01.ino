...
static String         Date_str, Time_str, Daily_Tracking_Date;
...
static float          bme_temp_c, bme_pres_hpa, daily_min_temp_c, daily_max_temp_c;
static bool           daily_range_initialised = false;
static bool           bme_sensor_ok = false;
...
void setup() {
  ...
  display.flipScreenVertically();                // In my case flip the screen around by 180 degrees
  ...
  bme_sensor_ok = bme.begin(0x76);                // For Adafruit sensors use address 0x77, for most 3rd party types use address 0x76
  if (!bme_sensor_ok) {
...
void loop() {
  ...
  UpdateDailyTemperatureRange();
  int display_page = (millis() / 4000) % 3;
  if (display_page == 1) {
    DrawStatusPage();
    delay(500);
    return;
  ...
  display.drawString(2, 20, String(bme_temp, 1) + (Format=="M"||Format=="X" ? " C" : " F"));                              // Display temperature in C (M/X) or F (I)
  ...
  //display.drawString(62,52, String(Calculate_WindDirection(), 0) + " deg");                                            // Display Wind direction
...
void BME280_Read_Sensor() {
  ...
  bme_pres_hpa = bme.readPressure() / 100.0F + pressure_offset;
  bme_temp = (Format == "I") ? bme_temp_c * 9.00F / 5.00F + 32 : bme_temp_c;
  bme_pres = (Format == "I") ? bme_pres_hpa / 33.863886666667F : bme_pres_hpa;
  bme_humi = bme.readHumidity();
}

void UpdateDailyTemperatureRange() {
  if (!daily_range_initialised || Date_str != Daily_Tracking_Date) {
    Daily_Tracking_Date = Date_str;
    daily_min_temp_c = bme_temp_c;
    daily_max_temp_c = bme_temp_c;
    daily_range_initialised = true;
    return;
  }
  daily_min_temp_c = min(daily_min_temp_c, bme_temp_c);
  daily_max_temp_c = max(daily_max_temp_c, bme_temp_c);
}

String GetWeatherStatus() {
  if (bme_pres_hpa >= 1020.0F) return "Sunny";
  if (bme_pres_hpa < 1005.0F) return "Rain Possible";
  return "Normal";
}

String GetComfortStatus() {
  if (bme_temp_c < 18.0F) return "Cold";
  if (bme_temp_c > 32.0F) return "Hot";
  if (bme_temp_c <= 28.0F && bme_humi <= 70.0F) return "Comfortable";
  return "Warm";
}

float DisplayTemperature(float temperature_c) {
  return (Format == "I") ? temperature_c * 9.0F / 5.0F + 32.0F : temperature_c;
}

void DrawPageHeader() {
  display.clear();
  display.setFont(ArialMT_Plain_10);
  display.drawString(0, 0, Date_str);
  display.drawString((Format == "I" ? 68 : 85), 0, Time_str);
  display.drawLine(0, 12, 128, 12);
}

void DrawStatusPage() {
  DrawPageHeader();
  display.drawString(0, 17, "Weather Status");
  display.setFont(ArialMT_Plain_16);
  display.drawString(0, 28, GetWeatherStatus());
  display.setFont(ArialMT_Plain_10);
  display.drawString(0, 49, "Comfort: " + GetComfortStatus());
  display.display();
}

void DrawDailyRangePage() {
  DrawPageHeader();
  display.drawString(0, 17, "Daily Temp Range");
  display.setFont(ArialMT_Plain_16);
  display.drawString(0, 30, "Min " + String(DisplayTemperature(daily_min_temp_c), 1) + (Format == "I" ? " F" : " C"));
  display.drawString(0, 47, "Max " + String(DisplayTemperature(daily_max_temp_c), 1) + (Format == "I" ? " F" : " C"));
  display.display();
}

void DrawBME280ErrorPage() {
  display.clear();
  display.setFont(ArialMT_Plain_16);
  display.drawString(0, 0, "Sensor Error");
  display.setFont(ArialMT_Plain_10);
  display.drawString(0, 24, "BME280 not found");
  display.drawString(0, 38, "Check SDA/SCL wiring");
  display.drawString(0, 52, "Expected addr: 0x76");
  display.display();
}
